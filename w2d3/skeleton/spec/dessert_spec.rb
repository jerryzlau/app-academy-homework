...
describe Dessert do
  ...
  let(:chef) { double("chef", name: "Jerry") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new('cake', '10', 'Jerry')}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Jerry")
      expect(cake.serve).to eq("Chef Jerry has made 10 cakes!")
    end
  end
